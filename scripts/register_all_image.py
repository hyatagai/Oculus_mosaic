...
if __name__ == "__main__":
    ...
    path_list.sort()
    save_path = ("/home/umigame/seatec/multibeam_image/2018-10-04-14-58-00/registered/")
    ...
    # if img_plr.shape[0] != 614:
        ...
        img_ctn_margined = cv2.warpAffine(img_ctn, np.float32([[1, 0, rotate_shift_x], [0, 1, 0]]), (rows*2, rows*2))
        rotate_matrix = cv2.getRotationMatrix2D((rows, rows), nexttheta + 90, 1)
        rotated_img_ctn = cv2.warpAffine(img_ctn, rotate_matrix, (rows*2, rows*2))
        translation_matrix = np.float32([[1, 0, nextx + (offset_cols - cols) / 2 - rotate_shift_x], [0, 1, nexty + offset_rows - rows * 1.5]])
        result_img = cv2.warpAffine(rotated_img_ctn, translation_matrix, (offset_cols, offset_rows))
